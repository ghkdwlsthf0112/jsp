...
<body>
	...
	<ul>
		...
		<li>뷰 쪽에서 자주 쓰인느 자바 문법들을 html 태그와 유사한 형식으로 사용할 수 있는 기능을 제공한다</li>
		<li>자바의 모든 기능을 사용할 수 있는 </li>
	</ul>
	...
	<dl>
		<dt>c:set</dt>
		<dt>각 영역에 aetAttribute를 편리하게 단축하여 사용할 쑤 있는 태그</dt>
		<dd>if문을 간편하게 쓸 수 있는 태그. else if와 else는 없다</dd>
		<dt>c:choose, c:when, c:other</dt>
		<dd>if, else if, else처럼 사용할 수 있는 태그</dd>
		<dt>c:forEach</dt>
		<dd>for문을 사용할 수 있는 태그.
			숫자를 통해 반복할 수도 있고 순서대로 꺼내며 반복할 수도 있다</dd>
		<dt>c:forTokens</dt>
	...
	<c:set var="snack" value="치토스"/> <!-- 스코프  -->
	
	<c:set var="coffee" value="초코라떼" scope="application"/>
	
	<h5># JSP 페이지에서 어트리뷰트를 쉽게 꺼내 쓸 쑤 있는 기을 제공한다</h5>
	...
	<div>aap : ${applicationScope.snack}</div>
	...
	<c:forEach begin="0" end="9" var="i">
		<li id="list-item${i}">${i}번째 리스트 데이터입니다</li>
	</c:forEach>
	</ul>
	...
	<%
		String name = "김철수";
		String[] coffees = {"Americano", "cafe Latte", 
				"Americano", "cafe Mocha", "카라멜 마끼아또"};
	...
	<c:forEach items="${foods}" var="food" varStatus="v">
		<c:choose>
			<c:when test="${v.first}">
				<div class="first">${v.index}번재 음식: ${food} (${v.count}개)</div>
			</c:when>
			<c:when test="${v.last}">
				<div class="last">${v.index}번재 음식: ${food} (${v.count}개)</div>
			</c:when>
			<c:otherwise>
				<div class="mid">${v.index}번재 음식: ${food} (${v.count}개)</div>
			</c:otherwise>
		</c:choose>
	</c:forEach>
	...
	<c:forTokens items="${animal}" delims="/#" var="animal">
		<div>${animal}</div>
	</c:forTokens>
