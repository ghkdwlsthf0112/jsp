...
<body>
	...
	<ul>
		<li>HTTP는 응답를 하고나면 클라이언트와의 연결을 끊어버린다</li>
	...
	<ol>
		<li>클라이언트가 최초로 웹 서버에 접속허면 서버는 해당 클라이언트에게 세션ID를 발급한다</li>
		<li>발급받은 세션ID는 클라이언트측의 웹 브라우저에 저장된다
			(같은 IP의 다른 웹 브라우저로 접속하면 새로운 세션IP를 발급받는다)</li>
		<li>이후 해당 클라이언트는 해당 서버로 요청을 보낼때마다 세션ID를 
			요청에 함께 실어 전달하게 된다</li>
		<li>서버는 실려온 세션ID로 클라이언트를 구분할 수 있다</li>
		<li>세션ID는 개인 사물함 열쇠 같은 역할을 하기도 한다.
			세션 어트리뷰트에 접근할 때 해당 세션ID를 열쇠로 사용한다.</li>
		<li>세션 객체에 너무 많은 양의 데이터를추가하는 것은
